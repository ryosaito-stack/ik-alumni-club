...
import { useAdminNewsletterMutations } from '@/hooks/newsletters/admin';
...
export default function NewNewsletterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // issueNumberが0の場合は自動採番される
    if (newsletterData.issueNumber === 0) {
      delete (newsletterData as any).issueNumber;
    }

    const id = await createNewsletter(newsletterData);
    
    if (id) {
      alert('ニュースレターを作成しました');
      router.refresh();
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-gray-900">ニュースレター新規作成</h1>
          <Link
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white shadow px-6 py-8 rounded-lg">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            第○号の番号を入力。0を入力すると最新号数+1が自動的に設定されます
          </p>
        </div>
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {uploadingPdf ? 'アップロード中...' : loading ? '作成中...' : '作成'}
          </button>
        </div>
      </form>
    </div>
