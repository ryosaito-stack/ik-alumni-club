...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    if (blog) {
      ...
      setOriginalThumbnailUrl(blog.thumbnail);
    }
  }, [blog]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (thumbnailFile) {
      ...
      try {
        ...
        uploadedThumbnailUrl = await uploadImage(thumbnailFile, 'blogs');
      } catch (err) {
  ...
  if (!blog) {
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center">ブログ記事が見つかりません</div>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-gray-900">ブログ編集</h1>
          <Link
            href="/admin/blogs"
        ...
        <p className="mt-2 text-sm text-gray-600">
          ...
          作成日: {new Date(blog.createdAt).toLocaleDateString('ja-JP')} | 
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white shadow px-6 py-8 rounded-lg">
        ...
        <div>
          ...
          <textarea
            ...
            className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            placeholder="記事の概要を入力してください（一覧表示に使用されます）"
            required
          />
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            HTMLタグが使用できます（例: &lt;p&gt;, &lt;br /&gt;, &lt;h3&gt;, &lt;ul&gt;, &lt;li&gt;, &lt;strong&gt;, &lt;a href=""&gt;）
          </p>
        </div>
